                        ...
                        <div className="relative">
                          ...
                          <div className="flex gap-1">
                            <input
                              ...
                              required
                              value={recipientName}
                              onChange={(e) => { setRecipientName(e.target.value); setRecipOpen(true); }}
                              onFocus={() => setRecipOpen(true)}
                              onBlur={() => setTimeout(() => setRecipOpen(false), 200)}
                              placeholder="აირჩიეთ მიმწოდებლებიდან ან ჩაწერეთ ხელით"
                              className="flex-1 px-2.5 py-1.5 bg-white rounded-xl border border-slate-200 text-xs text-slate-700"
                            />
                            <button
                              type="button"
                              title="მიმწოდებლებიდან არჩევა"
                              onMouseDown={(e) => e.preventDefault()}
                              onClick={() => setRecipOpen((o) => !o)}
                              className="px-2.5 bg-slate-100 hover:bg-slate-200 rounded-xl text-slate-500 text-xs font-bold"
                            >
                              ▾
                            </button>
                          </div>
                          {recipOpen && (
                            <div className="absolute z-30 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                              <div className="px-2.5 py-1.5 text-[10px] text-slate-400 border-b border-slate-50 sticky top-0 bg-white">
                                მიმწოდებლები · ძებნა ველში წერით
                              </div>
                              {suppliers
                                .filter((s) => s.name.toLowerCase().includes(recipientName.toLowerCase()))
                                .map((s) => (
                                  <button
                                    key={s.id}
                                    type="button"
                                    onMouseDown={(e) => e.preventDefault()}
                                    onClick={() => { setRecipientName(s.name); setRecipOpen(false); }}
                                    className="w-full text-left px-2.5 py-1.5 text-xs hover:bg-indigo-50 flex items-center justify-between gap-2"
                                  >
                                    <span className="truncate font-semibold text-slate-700">{s.name}</span>
                                    {s.iban && <span className="text-[9px] text-slate-400 font-mono truncate">{s.iban}</span>}
                                  </button>
                                ))}
                              {suppliers.filter((s) => s.name.toLowerCase().includes(recipientName.toLowerCase())).length === 0 && (
                                <div className="px-2.5 py-2 text-[11px] text-slate-400">ვერ მოიძებნა — გააგრძელეთ ხელით ჩაწერა</div>
                              )}
                            </div>
                          )}
                        </div>
