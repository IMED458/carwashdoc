...
export default function ReportsPanel({
  ...
  // Helper for generating simulated Excel downloads
  const handleDownloadExcel = () => {
    alert('Excel (CSV) ანგარიში წარმატებით გენერირდა და ჩამოიტვირთა!');
  };
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {expenses.map(e => (
                      <tr key={e.id} className="text-slate-700">
                        ...
                        <td className="p-3 font-medium text-xs">{e.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {payments.map(p => (
                      <tr key={p.id}>
                        ...
                        <td className="p-3 text-center capitalize">{p.paymentMethod}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                    ...
                    {expenses.map(e => {
                      ...
                      return (
                        <tr key={e.id} className="bg-amber-50/10">
                          ...
                          <td className="p-3">{suppliers.find(s => s.id === e.supplierId)?.name}</td>
                          <td className="p-3">
                            <span className="px-2 py-0.5 bg-amber-100 text-amber-800 text-[10px] font-bold rounded-full">{e.status}</span>
                          </td>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 font-semibold">
                      ...
                      <th className="p-3 text-right">დარიცხული (Gross)</th>
                      <th className="p-3 text-right">საშემოსავლო (20%)</th>
                      <th className="p-3 text-right">გადასარიცხი (Net)</th>
